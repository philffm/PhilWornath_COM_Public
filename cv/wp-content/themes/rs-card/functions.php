...
if ( class_exists( 'woocommerce' ) ) {
	...
	function woocommerce_header_add_to_cart_fragment( $fragments ) {
        ...
        <a class="head-woo" href="<?php echo esc_url( WC()->cart->wc_get_cart_url() ); ?>">
...
function rs_card_filter_plugin_updates( $value ) {
...
add_filter( 'site_transient_update_plugins', 'rs_card_filter_plugin_updates' );
...
if ( ! function_exists( 'rs_card_setup' ) ) :
	...
	function rs_card_setup() {
		...
		add_image_size( 'rs-card-profile-photo', 299, 299, true );
		add_image_size( 'rs-card-profile-photo-2x', 598, 598, true );
		...
		function rs_card_post_thumbnail( $post_id, $image_id, $size = 'image', $class = '' )
		{
			...
			$thumbnail = wp_get_attachment_image_src( $thumb_id, $size )[0];
			$thumbnail_2x = wp_get_attachment_image_src( $thumb_id, $size . '-2x' )[0];
